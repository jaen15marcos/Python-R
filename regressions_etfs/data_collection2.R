...
process_refinitiv_data <- function(refinitiv, join_key) {
  refinitiv_join_key <- join_key[, c(1:2, 6)]
...
check_refinitiv_data <- function(map_refinitiv) {
  map_refinitiv %>%
    ...
    select(sym, listing_mkt, RIC, date, closing_price, refinitiv.closing_price) %>%
    mutate(date = as.Date(date)) %>%
    ...
    filter(error > 1) %>%
    select(sym, listing_mkt, RIC, error)
}
...
main_pipeline <- function(file_paths) {
  ...
  # Merge with IFS data
  map_refinitiv_ifs <- merge_map_refinitiv_ifs(map_refinitiv, ifs)
...
file_paths <- list(
  ...
  refinitiv_ts = 'path/to/refinitiv_full_ts_updated.csv',
  ifs = 'path/to/ifs.csv',
